    ...
    private suspend fun processOnlineStreaming(bmp: Bitmap, gray: FloatArray) {
        ...
        try {
            ConvertApi.liveOcrStream(jpeg, model, mode) { sentence ->
                // Commit the motion reference only after the server has really
                // produced output. Previously it was committed before the HTTP
                // call, so one timeout/empty response made this same scene look
                // "already handled" forever — most visible in Describe mode.
                if (!any) readGray = gray
                any = true
