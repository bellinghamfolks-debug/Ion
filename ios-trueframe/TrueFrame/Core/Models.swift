...
public struct LevelReading: Equatable, Sendable {
    public var rollDegrees: Double     // signed; + = clockwise from level
    public var pitchDegrees: Double     // signed; + = pointed up
    public init(rollDegrees: Double, pitchDegrees: Double) {
        self.rollDegrees = rollDegrees; self.pitchDegrees = pitchDegrees
    }
...
public enum LevelThresholds {
    public static let level: Double = 1.0        // within ±1° reads as "level"
    public static let nearLevel: Double = 3.0    // within ±3° reads as "almost level"
    /// Smallest roll change worth re-announcing (avoids chatter).
    public static let announceStep: Double = 3.0
}

// MARK: - Per-frame vision analysis

public enum Sharpness: String, Sendable { case sharp, slightlySoft, blurry, severelyBlurry
...
public enum ExposureState: String, Sendable { case good, dark, veryDark, bright, overexposed
    public var spokenLabel: String {
        switch self {
...
public enum ObstructionRegion: String, Sendable {
    ...
    public var spoken: String {
        switch self {
        case .lowerLeft: return "lower-left"; case .lowerRight: return "lower-right"
        case .upperLeft: return "upper-left"; case .upperRight: return "upper-right"
        case .center: return "center"; case .unknown: return "part"
        }
    }
}
...
public struct ObstructionResult: Sendable {
    ...
    public init(isObstructed: Bool = false, region: ObstructionRegion = .unknown, confidence: Double = 0) {
        self.isObstructed = isObstructed; self.region = region; self.confidence = confidence
    }
}

/// Subject framing summary. Boxes are in normalized 0..1 coordinates, origin
/// top-left. No identity is computed — composition only.
public struct FramingResult: Sendable {
    public var personCount: Int
    public var faceCount: Int
    public var primarySubject: CGRect?     // largest person/face, normalized
    public var clippedTop: Bool
    public var clippedBottom: Bool
    public var clippedLeft: Bool
    public var clippedRight: Bool
    public init(personCount: Int = 0, faceCount: Int = 0, primarySubject: CGRect? = nil,
                clippedTop: Bool = false, clippedBottom: Bool = false,
                clippedLeft: Bool = false, clippedRight: Bool = false) {
        self.personCount = personCount; self.faceCount = faceCount
        self.primarySubject = primarySubject
        self.clippedTop = clippedTop; self.clippedBottom = clippedBottom
        self.clippedLeft = clippedLeft; self.clippedRight = clippedRight
    }
    public var hasClipping: Bool { clippedTop || clippedBottom || clippedLeft || clippedRight }

    /// Horizontal offset of the primary subject from center, in [-1, 1].
    public var horizontalOffset: CGFloat? {
        guard let s = primarySubject else { return nil }
        return (s.midX - 0.5) * 2
    }
}

/// Everything the guidance loop knows about the current preview frame.
public struct FrameAnalysis: Sendable {
    public var level: LevelReading
    public var sharpness: Sharpness
    public var motionHigh: Bool          // camera shaking (device motion magnitude)
    public var exposure: ExposureState
    public var obstruction: ObstructionResult
    public var framing: FramingResult
    public var skyFraction: Double        // 0..1 of frame that reads as bright sky
    ...
    public init(level: LevelReading,
        ...
        self.level = level; self.sharpness = sharpness; self.motionHigh = motionHigh
        self.exposure = exposure; self.obstruction = obstruction; self.framing = framing
        self.skyFraction = skyFraction; self.groundFraction = groundFraction
    }
}

// MARK: - Correction

/// A planned, non-generative correction. Angles are what will be applied; the
/// crop is the resulting valid region in the source's normalized space.
public struct CorrectionPlan: Equatable, Sendable {
    public var rollDegrees: Double                 // rotation to apply (signed)
    ...
    public var horizonConfidence: Double           // 0..1
    ...
    public var isIdentity: Bool {
        abs(rollDegrees) < 0.05 && abs(perspectiveVerticalDegrees) < 0.05 &&
        abs(perspectiveHorizontalDegrees) < 0.05 && cropNormalized == .full
    }
}
