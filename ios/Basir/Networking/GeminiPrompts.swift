...
enum GeminiPrompts {
    ...
    static func documentPageInstruction(langName: String,
        ...
        p += "right-to-left). Keep the original sequence of blocks UNLESS a more specific "
        p += "rule below applies (e.g. ordering transcript terms chronologically).\n"
        p += "COLUMNAR LAYOUT: a page may place several INDEPENDENT blocks SIDE BY SIDE "
        p += "(e.g. two or more academic terms in adjacent columns, or a multi-column form). "
        p += "Treat each block as an ISOLATED unit: read it completely — its heading and its "
        p += "WHOLE table, top to bottom — before moving to the next block. NEVER read "
        p += "straight across columns, and NEVER merge a row or header from one block with an "
        p += "adjacent block. This is critical: mixing adjacent blocks corrupts the data.\n"
        p += "ORIENTATION: a scanned page may be ROTATED (90°/180°) or skewed so the text "
        p += "runs sideways. Mentally rotate it and read it correctly regardless of "
        p += "orientation; never let rotation cause you to misread or guess.\n"
        p += "DO NOT FABRICATE — this is the most important rule. Transcribe ONLY what is "
        p += "actually printed and legible in the attached images. NEVER invent, guess, "
        p += "auto-complete, or 'fill in' any name, number, grade, course title, date, or "
        p += "row. If a value or region is blurry or unreadable, output \"[غير واضح]\" "
        p += "(or \"[unclear]\" in English) for that value — it is far better to mark "
        p += "something unclear than to write a plausible but wrong value. If a page has no "
        p += "real content, emit only its page_marker.\n"
        p += "COMPLETENESS: include every block, term, section, and table that is ACTUALLY "
        p += "present and legible on these pages, in source order — but never add ones that "
        p += "are not there.\n"
        ...
        p += "- ACADEMIC TRANSCRIPTS (any university, any grading system) — read with row "
        p += "integrity and self-validation:\n"
        p += "  • Each course row's code, name, credit hours, grade, points, and term GPA "
        p += "belong to the SAME physical row. Keep them together; NEVER shift a value up or "
        p += "down into a neighbouring row.\n"
        p += "  • INFER the institution's own grading scale from the document itself — do NOT "
        p += "assume a fixed one (it may be a 5-point, 4-point, percentage, or other system; "
        p += "grades may be Arabic letters, Latin letters, or numbers). A course's earned "
        p += "points = its credit hours × the grade's value on THAT document's scale. Use this "
        p += "to VERIFY each row: if a row's grade, hours, and points are not consistent with "
        p += "the scale the rest of the document uses, you misread something — re-read that row "
        p += "carefully and correct it so they agree.\n"
        p += "  • Order the terms CHRONOLOGICALLY (earliest academic year first, and earlier "
        p += "term before later term within a year).\n"
